...
- [x] 7.4 Wake POLL: `@Scheduled`+`@SchedulerLock` джоба (имя `poll-wake`, `lockAtMostFor` = `harness.lock.job-ttl`), eligible-скан `last_seq > last_consumed_seq`; тест «пропущенная EVENT-ом сессия подобрана POLL-ом»; чистка просроченных `sess-*`-строк отменена (D-46 — просроченные строки безвредны, джоба только «нашёл eligible → tryStart»)
...
- [x] 10.1 ArchUnit: границы модулей `identity/session/execution/intelligence/api` (кросс-импорты мимо контрактов запрещены); тест в CI
- [x] 10.2 Приёмочный end-to-end (критерий `roadmap.md` M1): FREE-сессия через минимальный attach (SSE+POST): сообщение → модель (WireMock) → инструменты в helper-контейнере (реальный Docker: write_file→bash→read_file) → стриминг; зелёный в CI
- [ ] 10.3 ~~Рестарт-устойчивость kill -9~~ — **отменена владельцем**: рестарт-поведение покрыто пост-краш-тестами рестарт-скана (пачка D, 7.6); kill -9 живого процесса в автотесте не воспроизводится честно
- [x] 10.4 Полный прогон `mvn verify` + сводка покрытия/результатов в `openspec/changes/m1-session-core/` (apply-заметки)
