...
- Существующий код в основном не меняется; осознанное расширение scope после живого прогона 2026-09-24 (tasks 2.5–2.6): `WorkspaceContainerManager.java` — текст ошибки подготовки workspace дополнен типом корневой причины (диагностика отказа, поведение не меняется) плюс регрессионная проверка в тесте; host-grants (владение корнем workspace, доступ к `/var/run/docker.sock` через `HARNESS_DOCKER_GID`/`group_add`) — в compose и README (D-R6).
