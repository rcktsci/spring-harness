...
**Решение**: compose продолжает собирать образ локально (`build:` + `image: spring-harness:local`, deployment-readme D-R2) — это быстрый путь для владельца и чистый checkout. Для CI-образа README получает короткий блок: `docker pull ghcr.io/rcktsci/spring-harness-orchestrator:latest` и `docker tag ... spring-harness:local` (либо замена строки `image:` в compose на GHCR-ссылку). Видимость пакета GHCR по умолчанию приватная: для pull нужен PAT (`read:packages`) либо публичная видимость пакета — решение владельца, README описывает оба варианта.
...
- [Архитектура VM окажется aarch64] → задача в tasks: проверить `uname -m` на VM; при `aarch64` вернуться к D-CI4 (arm64-вариант базовых образов может отсутствовать — проверить до правки).
...
- [Первый push в GHCR создаёт приватный пакет, pull с VM без PAT не работает] → README описывает оба варианта (сделать публичным / PAT `read:packages`); это разовая настройка владельца.
...
2. Владелец один раз: настроить видимость пакета GHCR (публичный или PAT для VM).
...
- Требуют подтверждения владельцем, но не меняют спеки/подход: (1) оставляем пакет GHCR приватным с PAT или делаем публичным; (2) подтверждаем x86_64 у VM (см. Risks).
