...
- **Внешние системы**: GitHub Container Registry (GHCR): первый push создаёт пакет с приватной видимостью (настройки репо видимость пакета не меняют) — владелец один раз решает: сделать пакет публичным либо держать приватным и заходить с VM по PAT (`read:packages`); способ логина и потребления образа — в README.
