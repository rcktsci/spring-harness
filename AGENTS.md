...
- **Пост-M5 харденинг по живому стенду завершён** (6 заархивированных change + D-94/D-95): `2026-09-25-relay-connect-consent-confirm` (авто-подключение и регистрация реле при открытии FREE-сессии, UI per-session consent, UI подтверждения каждой команды D-93, переписанный electron-e2e), `2026-09-25-desktop-ui-robustness` (disconnect терминирует регистрацию, at-most-once на ответы диалогов без буферизации в main, ошибка каталога агентов больше не выглядит как «агентов нет»), `2026-09-25-deployment-readme` (деплой с нуля: host-гранты, D-95), `2026-09-25-silent-refresh-single-flight` (один silent refresh на пачку запросов + эпоха сессии: logout/смена issuer больше не «воскрешаются» поздним ответом; терминальные/временные исходы refresh разделены; тела ответов KC санитизируются в логах), `2026-09-25-relay-session-workspace-isolation` (каталог выбирается строго по `call.sessionId` — команды сессии не могут исполниться в workspace другой сессии; таймауты KC и таймер регистрации не съедаются ожиданием consent), `2026-09-25-login-navigation-after-auth` + `2026-09-25-session-loss-ui-state` (после входа — сразу чат; непроизвольная потеря сессии приводит UI к экрану логина). Live-подтверждено на VM владельца: вход с нуля → чат → локальный `bash` в каталоге своей сессии после подтверждения.
- **CI публикации backend-образа в GHCR** (заархивирован: openspec/changes/archive/2026-09-26-ci-backend-image/; спеки `openspec/specs/backend-image-ci/` и `openspec/specs/release-policy/`): workflow `.github/workflows/backend-image.yml` по push в `main` (и тегам `v*`, и `workflow_dispatch`) собирает `docker/Dockerfile.orchestrator` в контейнере (раннеру нужен только docker) и публикует `ghcr.io/rcktsci/spring-harness-orchestrator` — на ветке теги `latest`/`main`/`sha-<short>`, на теге `vX.Y.Z` — `vX.Y.Z`/`X.Y.Z`/`sha-<short>`; `.dockerignore` держит контекст чистым (`api/` и `src/` не исключены — openapi-generator). Релизная политика SemVer (D-96): одна версия на backend и web-desktop, истина — git-тег; релиз-коммит сводит `pom.xml` + `web-desktop/package.json` к версии тега, обновляет строку `image:` в `docker-compose.yml` и добавляет запись в `CHANGELOG.md` (Keep a Changelog, в корне); CI на теге сверяет версии и не пускает перезапись опубликованного тега. Развёртывание на VM идёт пином версионного тега образа в `docker-compose.yml` (живая копия compose на VM правится руками: `/opt/spring-harness` — не git-клон), без локальной сборки в compose (решение владельца 2026-09-25, supersedes прежний D-CI10 «pull + retag»). Первый релиз `v0.1.0` вышел (коммит `00b36f5`, тег `v0.1.0`) и live-подтверждён на VM: контейнер healthy, actuator `UP`, label'ы `version=v0.1.0`/`revision=00b36f5`, откат на `sha-2286cbe` проверен и возвращён на `v0.1.0`.
...
- [CHANGELOG.md](CHANGELOG.md) — журнал релизов (Keep a Changelog); первый релиз — `0.1.0`.
...
- [openspec/specs/](openspec/specs/) — заархивированные спеки (`desktop-shell`, `desktop-chat`, `desktop-relay-client`, `desktop-session-tree`, `desktop-artifacts`, `backend-image-ci`, `release-policy`).
- [openspec/changes/archive/2026-09-26-ci-backend-image/](openspec/changes/archive/2026-09-26-ci-backend-image/) — заархивированный change: CI backend-образа + релизная политика SemVer (D-CI1…D-CI16, D-96).
