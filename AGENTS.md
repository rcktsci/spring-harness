...
- **CI публикации backend-образа в GHCR** (openspec/changes/ci-backend-image/): workflow `.github/workflows/backend-image.yml` по push в `main` (и тегам `v*`, и `workflow_dispatch`) собирает `docker/Dockerfile.orchestrator` в контейнере (раннеру нужен только docker) и публикует `ghcr.io/rcktsci/spring-harness-orchestrator` — на ветке теги `latest`/`main`/`sha-<short>`, на теге `vX.Y.Z` — `vX.Y.Z`/`X.Y.Z`/`sha-<short>`; `.dockerignore` держит контекст чистым (`api/` и `src/` не исключены — openapi-generator). Релизная политика SemVer (D-96): одна версия на backend и web-desktop, истина — git-тег; релиз-коммит сводит `pom.xml` + `web-desktop/package.json` к версии тега и добавляет запись в `CHANGELOG.md` (Keep a Changelog, в корне); CI на теге сверяет версии и не пускает перезапись опубликованного тега. Развёртывание на VM идёт пином версионного тега образа в `docker-compose.yml`, без локальной сборки в compose (решение владельца 2026-09-25, supersedes прежний D-CI10 «pull + retag»). Первый релиз `v0.1.0` готовится (версии `0.1.0` в `pom.xml`, `web-desktop/package.json` и строке `image:` compose сведены релиз-коммитом; тег пушится после аппрува).
...
- [openspec/changes/ci-backend-image/](openspec/changes/ci-backend-image/) — активный change: CI backend-образа + релизная политика SemVer (D-CI1…D-CI16, D-96).
