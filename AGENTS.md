...
- Дизайн-базис завершён (D-01…D-46). **M1 «Ядро сессий» завершён и заархивирован** (2026-09-18-m1-session-core); спеки 6 capability — в openspec/specs/. 209 тестов зелёных.
- Contract-first отработан: спека заморожена (ревью-цикл) → генерация 7.25 → контроллеры на сгенерированных интерфейсах → e2e через сгенерированный клиент.
- Следующий шаг: **M2 Workflow-движок** — /opsx:propose (объём: docs/design/roadmap.md, контракты в openspec/specs/).
## Ключевые документы
