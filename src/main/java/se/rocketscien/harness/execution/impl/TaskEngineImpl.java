 ...
 * всегда null). После коммита — EVENT-wake (и task-terminal для терминальных целей) в шину.
 */
@Service
...
public class TaskEngineImpl implements TaskEngine {
    ...
    public Transition processTaskTransition(UUID taskId, String fromState, String toState,
        ...
        int updated = jdbcTemplate.update(CAS_UPDATE,
                toState,
                target.kind().name(),
     ...
     * следующего состояния (BASH), переоценка WAIT_TASKS-наблюдателей, bootstrap AGENT (пачка J).
