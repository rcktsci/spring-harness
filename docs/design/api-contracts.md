...
| `GET /api/v1/sessions/{id}/tree` | `?depth=` (по умолчанию всё поддерево) → `{ items: TreeNode[] }`; `TreeNode = { id, parentSessionId, kind, agent {key,rev}, runtimeStatus: IDLE|TURN_RUNNING|PARKED_ASYNC|PARKED_CLIENT, lastSeq, lastActivityAt, taskId?, stateCode? }` — «проваливание» в субагентов; `taskId`/`stateCode` присутствуют только у STATE-узлов; `statusProjection` подзадачи — отдельно через `GET /tasks/{id}` (см. §4) |

**SessionDto**: `id, kind: FREE|STATE, title, owner, agent {key, rev}, workspaceBinding { type: SERVER_DIR }, runtimeStatus: IDLE|TURN_RUNNING|PARKED_ASYNC|PARKED_CLIENT, lastTurnOutcome?: COMPLETED|FAILED|CANCELLED, lastSeq, lastActivityAt, createdAt`. У STATE-сессий дополнительно присутствуют `taskId`, `stateCode`, `parentSessionId` (через GET `…/tree`, не в самом `SessionDto`). `owner` — **username** владельца (`preferred_username` из JWT; не `keycloak_subject` — тот остаётся внутренней идентичностью `app_user`; резолв `owner_user_id → username` — забота серверного слоя). `PARKED_CLIENT` — **зарезервированное** значение enum (D-84): присвоение вне M4 (авто-парковки клиента нет; разрыв → вызовы `tool-not-available`/LOST; UI в M5 рендерит значение raw-строкой — отдельного сценария нет).

**MessageDto**: `id (ULID), seq, kind: USER|ASSISTANT|SYSTEM|TOOL_CALL|TOOL_RESULT|COMPACT|ASYNC_ACCEPTED, author? (username; только USER), payload, callId?, late?, tokens?, createdAt`. `ASYNC_ACCEPTED` — плейсхолдер «принято, в полёте» для async-capable инструментов (M3); финальным результатом не считается, закрывается поздним `TOOL_RESULT` (`late=true`) или синтетическим LOST. Сообщения неизменяемы: PATCH/DELETE не существует.
