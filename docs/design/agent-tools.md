...
> **Источник 5: Desktop-клиент (M5, D-80/D-85)**. Клиентский runtime-оверлей релея (см. §3b выше) реализован в `web-desktop` (Electron + Vue 3): исполнение локальных `bash`/`read_file`/`write_file`/`edit`/`glob`/`grep` в `basePath` сессии, без path-guard (D-88 owner-risk), по протоколу §5. Локальный toolset объявляется при WS-register (`client: { version, tools[] }`); manifest на сервере получается через `client-tools` aggregator. Один экземпляр оверлея живёт только в активной WS-сессии; субагенты (через parent-chain, D-84) наследуют toolset, не свою копию. Замена источника/удаление клиента — рестарт-скан + LOST-финализация in-flight вызовов.
