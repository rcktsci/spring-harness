...
| `spawn_subagent` | `(agent_key, prompt, params?)` | любая сессия | синхронный; дочерняя сессия (`parent_session_id`); завершение = «ход окончен И pending_tool_calls == 0»; финальный ответ → `TOOL_RESULT` родителю; отмена каскадом по поддереву; owner дочерней сессии наследуется от родительской. **Лимиты (анти-форк-бомба, конфиг)**: глубина ≤ `spawn.maxDepth` (дефолт 3), детей на сессию ≤ `spawn.maxChildrenPerSession` (дефолт 8), активных сессий на пользователя ≤ `session.maxActivePerUser` (дефолт 50); превышение → `TOOL_RESULT forbidden (limit)` |
...
Доступны только агентам, у которых `permissions_jsonb.metaTools = true` (роль оркестратора). Субъект прав — сессия агента; всё созданное наследует `owner_user_id` от неё (см. глоссарий, Session). **Делегирование**: эффективные права = пересечение четырёх множителей (владелец ∩ декларация агента ∩ родительская цепочка ∩ инициатор инструкции — глоссарий, «Делегирование прав в сессиях»). **Гейт мета-операций**: `metaTools` исполняются только при `instructionSource = USER` (ход инициирован сообщением человека, не tool-output/вебхуком) — оркестратор в сессии с `harness-admin`-владельцем правит любые workflow только по инструкции владельца; субагент не может заспавнить metaTools-агента с получением его прав.

| Инструмент | Сигнатура | Семантика |
|---|---|---|
| `create_workflow` / `edit_workflow` | `(key, name?, graph)` | создание (rev=1) — любому; **правка — по делегированию**: владелец key или `harness-admin` у инициатора инструкции, иначе `forbidden`; та же валидация графа, что у REST (`422 graph-invalid` агенту в тексте ошибки) |
