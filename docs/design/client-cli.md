...
> **SUPERSEDED (M4)**: attach-CLI отменён — первым клиентом будет **Web Desktop (Electron + VueJS) отдельным этапом** (см. `roadmap.md` M5). Документ сохранён как исторический черновик UX; WS-релей в M4 — server-side, клиентская часть — только тестовый WS-клиент.
